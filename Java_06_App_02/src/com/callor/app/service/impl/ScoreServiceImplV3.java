...
public class ScoreServiceImplV3 extends ScoreServiceImplV2 {
	...
	public void saveScore() {
		...
		/*
		 * BufferedOutput 사용 이유
		 * 
		 * 실제 데이터를 파일에 기록하기 위해서는 FileOutputStream만 있어도 되지만
		 * 파일에 기록하고 저장하는 일은 상당히 많은 시간을 소모한다
		 * 컴퓨터 메모리, CPU 입장에서는 파일을 기록하는 동안 아무것도 하지 못하고
		 * 기다려야 하는 상황이 발생한다
		 * 이때 중간에 BuffereOutput이라는 파이프를 연결해두고
		 * 애플리케이션에서는 BufferOutput에게 데이터를 모두 보내고 다른 일을 수행한다
		 * 그러면 BuffereOutput과 FileOutpur이 협력하여 데이터를 파일에 기록하는 일을 대신 수행해준다
		 */
		...
		try {
			...
			for(ScoreVO scVO : scList) {
				...
				writeStr += String.format("%d : ", scVO.getKor());
				writeStr += String.format("%d : ", scVO.getEng());
